...
Pull Request の Files changed を開くと、キーワードに一致したファイルが自動で Viewed になり折りたたまれます。差分が遅延読み込みされても、読み込まれたタイミングで処理されます（画面のスクロール位置を勝手に動かすことはありません）。
...
走査は「ファイルパス起点」と「Viewed ボタン起点」の結果を統合し、どちらか一方でしか拾えないファイル（パス属性を持たないもの、ボタンだけが先に描画されるものなど）も取りこぼさないようにしています。両方が空の場合はファイルの囲みを起点に走査します。セレクタは `src/content/selectors.js` に集約しているので、動かなくなったときはこのファイルに候補を足すだけで対応できます。
...
1. ツールバーのアイコンを開き、状態表示を見る
   ...
   - 「未認識 n 件」→ ファイル一覧の数より走査できた数が少ない。走査漏れなので `tools/diagnose-dom.js` の `loadingHints` を確認する
