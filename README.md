...
| 遅延読み込みされた差分も処理する | する | 下記参照 |
| 連続クリックの間隔 | 120ms | ファイル数が多い PR で保存が追いつかない場合に増やす |

### 遅延読み込みされた差分

GitHub は Pull Request の差分を一度に描画せず、画面に近づいたファイルから順に読み込みます（`progressive-diffs-list`）。DOM に無いファイルは操作できないため、28 ファイルの PR を開いても最初は 10 ファイルほどしか対象になりません。

**遅延読み込みされた差分も処理する** を有効にすると、ページを少しずつスクロールしながら、DOM に載った分をその都度処理します。終わると元のスクロール位置に戻ります。画面外を DOM から外す実装（仮想化）でも取りこぼしません。

- スクロール走査は 1 ページにつき 1 回だけ実行します（手動実行では毎回）
- オフにすると、画面に出ている分だけが対象になります。その場合も自分でスクロールすれば、読み込まれた分は自動で処理されます
- ポップアップの状態表示に「未読み込み ○ 件」が出ます
...
src/content/              ページ側の処理
  ...
  toggle-state.js         checkbox / トグルボタンの状態差を吸収
  diff-loader.js          遅延読み込みされた差分をスクロールで DOM に載せる
